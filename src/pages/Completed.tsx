...
const Completed = () => {
  ...
  const filteredTasks = useMemo(async () => {
    if (searchQuery.trim()) {
      ...
      return allSearchResults.filter(task => task.status === 'completed');
    }
    return completedTasks;
  }, [completedTasks, searchQuery, searchTasks]);
        ...
        <main className="container max-w-screen-2xl px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
          <div className="mb-6 lg:mb-8">
            ...
            <p className="text-muted-foreground font-inter text-sm sm:text-base">
              {displayedTasks.length} completed task{displayedTasks.length !== 1 ? 's' : ''}
